...
function updateRelocalizationStatus() {
  const hasDirectMapBody = Boolean(
    getTfTransform("map", "body") ||
    getTfTransform("map", "base_link") ||
    getTfTransform("map", "base_footprint")
  );

  const hasFastLioChain = Boolean(
    getTfTransform("map", "camera_init") &&
    (
      getTfTransform("camera_init", "body") ||
      getTfTransform("body", "base_link") ||
      getTfTransform("body", "base_footprint")
    )
  );

  const hasOdomChain = Boolean(
    getTfTransform("map", "odom") &&
    (
      getTfTransform("odom", "base_link") ||
      getTfTransform("odom", "base_footprint")
    )
  );

  setRelocalizationStatus(hasDirectMapBody || hasFastLioChain || hasOdomChain);
}
...
function resolveRobotPose() {
  ...
  const directMapBase =
    ...
    findTransformByChildPrefixMatch("map", ["body", "odin1_base_link", "robot_base_link", "base_link", "base_footprint"]);
  if (directMapBase) {
  ...
  const odomToBase =
    ...
    findTransformByChildPrefixMatch("odom", ["body", "odin1_base_link", "robot_base_link", "base_link", "base_footprint"]);
